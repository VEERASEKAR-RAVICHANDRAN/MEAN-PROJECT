...
const ProductSchema = new mongoose.Schema({
    ...
    image: { type: String }, // This will store the Base64 string
    description: { type: String },
    stock: { type: Number, required: true },
    mealType: { type: String, required: true }, // New field: Breakfast, Lunch, Dinner
    foodType: { type: String, required: true }, // New field: Veg, Non-Veg
  });
...
app.post('/api/products', upload.single('productImage'), async (req, res) => {
    try {
      const { name, category, price, description, stock, mealType, foodType } = req.body;
  
      // Validate required fields
      if (!name || !category || !price || !stock || !mealType || !foodType) {
        return res.status(400).json({ error: 'Invalid input. Please check the provided data.' });
      }
      ...
      const product = new Product({
        ...
        description,
        stock,
        mealType, // Add meal type
        foodType, // Add food type
        image: productImage,
      });
